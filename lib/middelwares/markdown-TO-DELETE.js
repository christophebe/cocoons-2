      ...
      if (extension === util.HTML_EXTENSION) {
        ...
        fs.exists(newPath, function(exists) {
          if (exists) {
            ...
            renderMarkdown(req, res, next, newPath);
          } else {
                      //TODO : custom plugin ?
...
function renderMarkdown(req, res, next, file) {

  req.app.markdownPage.buildFromFile(file, function(error, page) {
    ...
    req.app.widgetRenderer.renderWidgets(params, function(error, htmlWidgets) {
      if (error) {
        ...
        page.content = new req.app.markdownRender.HTMLRenderer(params).renderToHTML(page.content);
