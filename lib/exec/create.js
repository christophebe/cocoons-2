...
const stat   = blue.promisify(require("fs").stat);
...
async function createSite(siteTemplateName, websiteFolder = process.cwd()) {
  ...
  return Promise.all([t, w]);


}

async function createTemplateDirectory(siteTemplateFolder, websiteFolder, options) {
  log.info("Use site template in : " + siteTemplateFolder);
  await fs.copy(siteTemplateFolder, websiteFolder, options);
}
...
async function createWidgetDirectory(websiteFolder, options) {
  ...
  await fs.copy(widgetTemplateFolder, websiteFolder + "/" + DEFAULT_WIDGET_FOLDER , options);
}
