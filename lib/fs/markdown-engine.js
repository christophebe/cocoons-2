...
/**
 *  Module used to transform markdown content into HTML
 *  based on plug templates
 */
...
/**
 * Render an url/resouce that match to a markdown content
 * into an HTML page
 *
...
async function renderMarkdown(req, res, file) {
  try {
    ...
    const [widgets, content] = await Promise.all([pWidget(params, req.app.widgetRenderer), pContent(params, page.content, req.app.markdownRender)]);
