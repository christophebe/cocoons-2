...
var jsonfile = blue.promisifyAll(require("jsonfile"));
var pug      = require('pug');
...
async function readJsonFile(jsonPath) {
    return await jsonfile.readFileAsync(jsonPath);
}
...
async function compilePug(pugFile) {
    return await fs.readFileAsync(pugFile, 'utf8')
         .then(fileContent => pug.compile(fileContent));
}
